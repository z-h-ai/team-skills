...
1. **联网搜索法条**（使用 web_search + web_fetch）：
   ```bash
   # 搜索具体法条（避免 flk.npc.gov.cn，使用开放法律网站）
   web_search "劳动合同法 第47条 经济补偿" --count 3
   # 选择一个结果，用 web_fetch 抓取完整内容
   web_fetch "https://www.fadada.com/article/..."
   # 提取法条文本，通过环境变量传给脚本
   export AGENT_LAW_ARTICLES='["法条1完整内容", "法条2完整内容", "法条3完整内容"]'
   ```

2. **运行生成脚本**：
   ```bash
   cd scripts && python3 generate_posters.py
   ```
   脚本会自动：
   - 读取 Agent 搜索的法条（优先）或使用内置法条
   - 并行调用 Gemini API 生成 3 张图片（加速）
   - 输出 `SEND_TO_FEISHU:` 标记

3. **自动发送到飞书**：
   - Agent 识别 `SEND_TO_FEISHU:` 标记
   - 使用 `message` 工具发送图片到当前对话
