...
AGENT_LAW_ARTICLES = os.environ.get("AGENT_LAW_ARTICLES")
if AGENT_LAW_ARTICLES:
    try:
        import json as _json_env
        LAW_ARTICLES_CACHE = _json_env.loads(AGENT_LAW_ARTICLES)
        print(f"使用 Agent 搜索的法条: {len(LAW_ARTICLES_CACHE)} 条")
    except:
        LAW_ARTICLES_CACHE = None
else:
    LAW_ARTICLES_CACHE = None
...
print("正在获取今日法条...")
if LAW_ARTICLES_CACHE:
    law_articles = LAW_ARTICLES_CACHE[:3]
    print(f"使用 Agent 预搜索的 {len(law_articles)} 条法条")
else:
